...
const ClientSideLivenessDetector = ({ onComplete }) => {
  ...
  const processChallenge = async (challengeIndex) => {
    setLoading(true);
  ...
  const completeVerification = (success) => {
    ...
    if (onComplete) {
      onComplete(success, capturedFrames);
    }
  };

  // Verify a specific challenge using face-api.js
  const verifyChallenge = async (action, frames) => {
    try {
      switch (action) {
        case "face_detection":
  ...
  const verifyFaceDetection = async (frameData) => {
    try {
  ...
  const verifyBlink = async (frameDataArray) => {
    try {
  ...
  const verifyHeadTurn = async (frameData) => {
    try {
      const img = await createImageFromDataUrl(frameData);
      const detections = await faceapi
        .detectAllFaces(img, new faceapi.TinyFaceDetectorOptions())
  ...
  const verifySmile = async (frameData) => {
    try {
      const img = await createImageFromDataUrl(frameData);
      const detections = await faceapi
        .detectAllFaces(img, new faceapi.TinyFaceDetectorOptions())
  ...
  return (
    <div className="client-liveness-detector">
